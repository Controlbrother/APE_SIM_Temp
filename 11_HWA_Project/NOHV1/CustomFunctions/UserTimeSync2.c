...
#define DS_BUS_CUSTOM_FEATURE_NAME usertimesync2           
/* End of feature parameter definition */

#include <stdio.h>
#include <stdlib.h>
#include <DsBusCustomCode_PduUserCode.h>
// #include <DsBusCustomCode_SecOC.h>
...
//extern uint8 CRC8H2F(uint8 *msg,uint8 ByteIndex_Stsrt,uint8 ByteIndex_End)
...
// printf("%s,,%s,%d",name,featureDescriptor,dataId);
	...
	if (isTx)
	{
		...
		if (userSYNCFLAG<1)//acc pdu        
		{
            ...
            printf("SyncTimeS: %lld",SyncTime_S);
