/*
 * LookUp_real_Treal32_T_real32_T.h
 *
 * Code generation for model "PubIfControlLONC_MDL".
 *
 * Model version              : 3.55
 * Simulink Coder version : 9.4 (R2020b) 29-Jul-2020
 * C source code generated on : Wed Mar  9 12:03:11 2022
 * Created for block: <S18>/VlcAxCtrlDevTresAddDC2EC
 */

#ifndef RTW_HEADER_LookUp_real_Treal32_T_real32_T_h_
#define RTW_HEADER_LookUp_real_Treal32_T_real32_T_h_
#include "rtwtypes.h"

void LookUp_real_Treal32_T_real32_T(real_T *pY, const real32_T *pYData, real32_T
  u, const real32_T *pUData, uint32_T iHi);

#endif                        /* RTW_HEADER_LookUp_real_Treal32_T_real32_T_h_ */
