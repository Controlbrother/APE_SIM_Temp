/*
 * look2_iu8fbfflftf_binlcpw.h
 *
 * Code generation for model "PubIfDecisionPLAN_MDL".
 *
 * Model version              : 1.99
 * Simulink Coder version : 9.4 (R2020b) 29-Jul-2020
 * C source code generated on : Wed Mar  9 12:04:00 2022
 * Created for block: PubIfDecisionPLAN_MDL
 */

#ifndef RTW_HEADER_look2_iu8fbfflftf_binlcpw_h_
#define RTW_HEADER_look2_iu8fbfflftf_binlcpw_h_
#include "rtwtypes.h"

extern real32_T look2_iu8fbfflftf_binlcpw(uint8_T u0, real32_T u1, const
  real32_T bp0[], const real32_T bp1[], const real32_T table[], const uint32_T
  maxIndex[], uint32_T stride);

#endif                             /* RTW_HEADER_look2_iu8fbfflftf_binlcpw_h_ */
