classdef TrajectoryGeneratorFrenet < handle
    ...
    methods
        function obj = TrajectoryGeneratorFrenet(FtNumTarpoints)
            obj.FtNumTarpoints = FtNumTarpoints;
        end
        % 横纵向同时规划
        function [frenetTrajectory,globalTrajectory,numTraj] = FrenetTrajectory(obj,ReferencePath,initialState,arcLenOffsets,lateralDeviation,LongTermState,LaterlTermState,timeSpan,FtNumTarpoints)
            ...
            for i = 1:numel(frenetTrajectory)
                ...
                globalTrajectory(i).Trajectory = ReferencePath.frenet2global(frenetTrajectory(i).Trajectory);
            end
%             globalTrajectory = obj.ReferencePath.frenet2global(frenetTrajectory);
        end
        % 单横向规划、规定好纵向规划距离
        function [frenetTrajectory,globalTrajectory,numTraj] = FrenetTrajectory_Lateral(obj,ReferencePath,initialState,arcLenOffsets,lateralDeviation,FtNumTarpoints,GtNumTarpoints) 
           ...
           globalTrajectory = repmat(struct('Trajectory',globaltraj,'costs',costs), numel(frenetTrajectory), 1);
            for i = 1:numel(frenetTrajectory)
                globalTrajectory(i).Trajectory = ReferencePath.frenet2global1D(frenetTrajectory(i).Trajectory,GtNumTarpoints); %此处GtNumTarpoints点数计算目标轨迹的世界坐标
            end
        end
    end
...
%              end
    ...
    methods(Static)
        ...
        function[InitialState,TerminalState,numTraj] = TrajStateCombination_Lateral(initialState,arcLenDeviation,lateralDeviation)
            ...
            numTraj = size(TerminalState,1);
            initialState(:,3) = pi/2 - initialState(:,3);% 此处注意坐标系，后来改为车辆坐标系，这个地方需要去除pi/2
            InitialState = repmat(initialState, numTraj,1);
        end
