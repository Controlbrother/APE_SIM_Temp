classdef FrenentReferencePath < handle
    ...
    properties(Access = public)
        ...
        DiscretizationDistance = single(1);    %离散距离
    ...
    methods(Access = public)
        function obj = FrenentReferencePath(GtNumTrajpoints)
            % Set waypoints
            obj.GtNumTrajpoints = GtNumTrajpoints;
            % obj.PathPoints = obj.generationCartesianStates(waypoints,GtNumTrajpoints);
        end  
        function generationCartesianStates(obj,waypoints,GtNumTrajpoints)
            ...
            persistent objSpline
            if isempty(objSpline)
                objSpline = Spline2D;
            end
            objSpline.Update_Spline2D(waypoints(:,1),waypoints(:,2));
...
%             end
        ...
        function [pathPoints, index] = interpolate(obj, arcLength)
            ...
            initIdx = index<=0;
            %pathPoints(initIdx,:) = repmat(obj.PathPoints(1,:),nnz(initIdx),1);%生成代码存在问题，暂时先注释，这种情况也很少出现。
            % 改成下边计算方式
            for i=1:length(index)
                if index(i)<=0
                    pathPoints(i,:)=obj.PathPoints(1,:);
                end
            end
            % Find valid indices
