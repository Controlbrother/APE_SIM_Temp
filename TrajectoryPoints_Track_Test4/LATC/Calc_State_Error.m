...
persistent ref_curK1
if isempty(ref_curK1)
    ref_curK1 = single(0);
end
while uint8(1)
    % 确定轨迹点间的步长
    ...
    % 为防止曲率突变，导致方向盘抖动，加预瞄点曲率跳变约束
    ref_curvature = GlobalTrajectory(1).Trajectory(idx,4);
    
    if abs(ref_curvature - ref_curK1) > 0.001
        L_Forward_Dis = max(single(0.8) * L_Forward_Dis,2*Step_temp);
    else
        break;
    end
end
